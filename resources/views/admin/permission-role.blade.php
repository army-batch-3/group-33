...
@section('modal')
<div class="modal fade" id="modal_permission" tabindex="-1" role="dialog" aria-labelledby="myModalLabel-2" style="display: none;">
	<div class="modal-dialog modal-sm" role="document" style="width: 500px;">
		<div class="modal-content" style="width: 515px;">
            ...
            <div class="modal-footer">
               ...
               <button type="button" class="btn btn-primary btn-sm waves-effect waves-light" id="store_permission">Save changes</button>
            </div>
		   </div>
	   </div>
   </div>
</div>
<div class="modal fade" id="modal_role" tabindex="-1" role="dialog" aria-labelledby="myModalLabel-2" style="display: none;">
	<div class="modal-dialog modal-sm" role="document" style="width: 500px;">
		<div class="modal-content">
            ...
            <div class="modal-footer">
               <button type="button" class="btn btn-default btn-sm waves-effect waves-light" data-dismiss="modal">Close</button>
               <button type="button" class="btn btn-primary btn-sm waves-effect waves-light" id="store_role">Save changes</button>
            </div>
		   </div>
	   </div>
   </div>
...
$(document).on('click',"#store_role",function(){
   ...
   $.ajax({
       ...
       data:({role_id:_roleid_assign, role_name:role_value, role_unset:role_unset}),
       type:"POST",
       dataType:'JSON',
   }).done(function(data){
      console.log(data);
...
$(document).on('click',"#store_permission",function(){
   ...
   $.ajax({
       ...
       data:({role_id:_roleid, permission_name:permission_value, permission_unset:permission_unset}),
       type:"POST",
       dataType:'JSON',
   }).done(function(data){
      console.log(data);
...
$(document).ready(function () {
   ...
   var role_table = $('#roles-table').DataTable({
         ...
         role_table.on( 'select', function () {
            btn_value_id = role_table.row( { selected: true } ).data().name;
      ...
      var user_table = $('#assign-table').DataTable({
         ...
         user_table.on( 'select', function () {
            btn_value_id = user_table.row( { selected: true } ).data().name;
